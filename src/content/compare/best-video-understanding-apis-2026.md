...
| **Primate Vision** | JEPA perception model → closed-vocabulary verdict + calibrated 0–1 confidence | ✅ Designed out — yes/no/indeterminate, never free-form generation | ✅ Managed WebRTC, 45ms p50 per frame | ✅ 108,000-frame signup grant (up to 30 h @ 1 fps), no card; instant sandbox key for agents | ✅ MCP, llms.txt, `POST /v1/sandbox` | $0.0000015/frame metered (≈16¢/hr @ 30 fps); enterprise plans for 24/7 |
...
**Pricing — two lanes, stated plainly.** Metered: **$0.0000015 per frame processed** (0.00015¢/frame — replacing our per-second model); queued time free, failed jobs free. You pay for the frames actually analyzed, so your capture rate sets your cost: an hour of 30 fps video ≈ 16¢, the same hour at 1 fps ≈ 0.5¢. That's the on-demand lane — clips, incidents, agent-invoked checks, bursty workloads. A 30-second clip in, a verdict with evidence out: a fraction of a cent. Camera fleets and 24/7 estates run on [enterprise plans](https://primateintelligence.ai/pricing#enterprise) — dedicated capacity, volume terms, and SLAs.
...
| **Primate Vision — metered** | per-frame, flat | $0.0000015/frame | **$0.16** (30 fps) † |
| **Primate Vision — enterprise** | contact us | custom | *a small fraction of the metered rate — the 24/7 / fleet lane* |

† *Metered rate normalized at 30 fps for comparison (≈0.5¢/camera-hour at 1 fps). Fleet and 24/7 workloads use [enterprise plans](https://primateintelligence.ai/pricing#enterprise). The metered lane's real unit is the answered question, not the camera-hour: a 30-second incident clip → verdict + confidence + evidence for a fraction of a cent.*
...
- **Fidelity parity changes the math.** The token rows sample ~1fps. At 12fps parity, Gemini's input tokens alone reach $0.28–$0.37/min — versus our all-in ≈0.1¢/min at 12 fps — before output tokens, and with no verdict layer attached.
